function fcd_ffdir -d "fuzzy find a directory, pass root dir and sequential search strings"
	...
	if test (count $args) -gt 0
		...
		set -l results (find -L "$new_path" -maxdepth $max_depth -mindepth 1 -regextype "posix-extended" -$case_sensitive ".*/[^.]*$regex.*" -type d 2> /dev/null)
		...
		if test -n "$found"
			...
			set results (find -L "$new_path" -maxdepth $max_depth -mindepth 1 -regextype "posix-extended" -$case_sensitive ".*$regex.*" -type d 2> /dev/null)
			if set -q _flag_shortest
